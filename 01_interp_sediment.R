...
pts$use <- 'yes'
pts$use[pts$tot!=100] <- 'no'

pts$year<-format(pts$time,'%Y')

write_sf(pts,'gis/seasketch/pts_bndo.fgb')

ggplot(pts,aes(x=year,fill=use))+geom_histogram(stat='count')+coord_flip()
